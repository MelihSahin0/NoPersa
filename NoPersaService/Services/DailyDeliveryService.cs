    ...
    public class DailyDeliveryService : IHostedService, IDisposable
    {
        ...
        public async Task CatchUp(NoPersaDbContext context, Maintenance maintenance)
        {
            ...
            try
            {
                ...
                List<Customer> dbCustomers = await context.Customers.Where(r => r.RouteId != long.MinValue || (r.RouteId == long.MinValue && r.MonthlyOverviews.Any(x => x.Year == maintenance.Date.Year && x.Month == maintenance.Date.Month)))
                ...
                {
                    Holiday? holiday = await context.Holidays.AsNoTracking().FirstOrDefaultAsync(h => h.Country.Equals(currentCountry) && h.Year == time.Year && h.Month == time.Month && h.Day == time.Day);
                    ...
                    foreach (Customer dbCustomer in dbCustomers)
                    {
                        ...
                        {
                            if (dbCustomer.RouteId != long.MinValue)
                            {
                ...
                (await context.Maintenances.FirstAsync(m => m.Id == 1)).Date = DateTime.Today.AddDays(1).Date;
        ...
        private void DoWork(object? state)
        {
            Task.Run(async () =>
            {
                using (var scope = serviceProvider.CreateScope())
                {
                    ...
                    var maintenance = await context.Maintenances.FirstOrDefaultAsync(m => m.Type == MaintenanceTypes.DailyDelivery.ToString());
                    if (maintenance != null)
                    {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            await context.Maintenances.AddAsync(new Maintenance() { Id = 1, Type = MaintenanceTypes.DailyDelivery.ToString(), Date = DateTime.Today.Date });
                            await context.SaveChangesAsync();
                            await transaction.CommitAsync();
                        }
                        catch (Exception ex)
        ...
        {
            if (!await context.Holidays.AsNoTracking().AnyAsync(h => h.Country.Equals(currentCountry) && h.Year == year))
            {
